...
//void i2c_setup();
/**
 * @brief Para que o mestre possa inciar uma transimissao � necessario
 * ele se certificar de que o barramento I2C n�o esteja envolvido em alguma transmissao
 */
void i2c_idle(void);

/**
 * @brief Habilita condicao de START(somente modo Mestre). A condicao
 * START � gerada quando o bit SEN � setado. O bit SEN � automaticamente pagado
 * pelo hardware no final da condi�ao START, sendo no mesmo momento setado o
 * bit SSPIF(Uma interrup�ao sera gerada se ela estiver habilitada).
 */
void i2c_start(void);

/**
 * @brief A fucao I2C_RESTART e exatamente igual a funcao I2C_START
 * A condicao START repetida ou Restart permite que seja alterada a dire�ao de fluxo
...
/**
 * @brief A fucao STOP � gerada quando o bit PEN � setado(inicia condicao
 * STOP nos pinos SDA e SCL). O bit PEN � automaticamente pagado pelo hardware no
 * final da condi�ao STOP, sendo no mesmo momento setado obit SSPIF(Uma interrup�ao
 ...
 * descida do nono pulso de clock, sendo gerado uma interrup�ao se ela estiver habilitada.
 */
void i2c_ack(void);
